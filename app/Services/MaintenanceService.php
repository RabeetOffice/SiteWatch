...
final class MaintenanceService
{
    ...
    /**
     * @return array{checks: int, activity: int, notifications: int, heartbeats: int, login_attempts: int, remember_tokens: int, duration_ms: int}
     */
    public function cleanup(): array
    {
        $started = microtime(true);
        $heartbeatId = $this->heartbeats->start('cleanup');
        $result = ['checks' => 0, 'activity' => 0, 'notifications' => 0, 'heartbeats' => 0, 'login_attempts' => 0, 'remember_tokens' => 0, 'duration_ms' => 0];
        ...
        try {
            ...
            $message = sprintf('Removed %d checks, %d activity entries, %d notifications, %d heartbeats.', $result['checks'], $result['activity'], $result['notifications'], $result['heartbeats']);
